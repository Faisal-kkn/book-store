...
import { searchBook } from "../Api/Requests";
...
const HomePage: React.FC = () => {
  ...
  const handleSubmit = async (e: any) => {
    try {
      ...
      if (search === null || regex.test(search)) {
        ...
        const response = await searchBook(search, 1);
        setCurrentPage(1)
        if (response.status !== 200) {
          notify("Network response was not ok.", 'error')
        } else {
  ...
  const searchMovie = async (value: number) => {
    ...
    try {
      const searchResponse = await searchBook(search, value);
      if (searchResponse.status !== 200) {
        notify("Network response was not ok.", 'error')
      } else {
        if (searchResponse?.data?.totalItems == 0) {
          setSearchError(true)
          setSearchResult({ totalItems: 0, items: [] })
          setTimeout(() => {
            setSearchError(false)
